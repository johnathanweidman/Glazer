...
function N(t, n, e, i) {
	return t[1] && i ? C(e.ctx.slice(), t[1](i(n))) : e.ctx;
}
function Y(t, n, e, i) {
	if (t[2] && i) {
		const r = t[2](i(e));
		...
		if (typeof r == 'object') {
			...
			for (let u = 0; u < c; u += 1) s[u] = n.dirty[u] | r[u];
			return s;
		}
		return n.dirty | r;
	}
...
function tt(t) {
	return t && D(t.destroy) ? t.destroy : w;
}
let m = !1;
function nt() {
	m = !0;
...
function O(t) {
	...
	for (let l = 0; l < n.length; l++) {
		const a = n[l].claim_order,
			o = (r > 0 && n[e[r]].claim_order <= a ? r + 1 : M(1, r, (S) => n[e[S]].claim_order, a)) - 1;
		i[l] = e[o] + 1;
		const k = o + 1;
	...
	for (let l = e[r] + 1; l != 0; l = i[l - 1]) {
		for (s.push(n[l - 1]); u >= l; u--) c.push(n[u]);
		u--;
	}
	for (; u >= 0; u--) c.push(n[u]);
	s.reverse(), c.sort((l, a) => l.claim_order - a.claim_order);
	for (let l = 0, a = 0; l < c.length; l++) {
		for (; a < s.length && c[l].claim_order >= s[a].claim_order; ) a++;
		const o = a < s.length ? s[a] : null;
		t.insertBefore(c[l], o);
	}
}
function T(t, n) {
	if (m) {
...
function it(t, n, e) {
	m && !e ? T(t, n) : (n.parentNode !== t || n.nextSibling != e) && t.insertBefore(n, e || null);
}
function rt(t) {
...
function F(t, n, { bubbles: e = !1, cancelable: i = !1 } = {}) {
...
function G() {
	...
	do {
		...
		for (p(null), d.length = 0, f = 0; E.length; ) E.pop()();
...
function jt(t) {
	const n = [],
		e = [];
	_.forEach((i) => (t.indexOf(i) === -1 ? n.push(i) : e.push(i))), e.forEach((i) => i()), (_ = n);
}
export {
	B as A,
	K as B,
